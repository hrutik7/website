...
Here we will see how to handle authentication in CI tests using [github actions](https://github.com/features/actions) . This approach allows to authenticate and test app easily.

_Requirements_ : We just need Supabase app using magic login for authentication. Here we are using Supabase CLI to start the database in CI rather than connecting the database

**Supabase local developement**

For reference you can refer the supbase docs [here](https://supabase.io/docs/guides/local-development)
...
Supabse sends the email asyncrhronously , there's a tiny delay , and we can't gurantee the time between triggering the email and when we try to finish logging in
