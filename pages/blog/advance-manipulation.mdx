...
Playwright manages cookies through the _browser context_. The browser context is a container for a set of pages that share the same cookies and storage. By default, Playwright creates a new browser context for different browser instances, allowing developers to manage cookies and storage independently for each context.
